...
function resolveEditorialScene({ riskLevel = 'none', hasReport = false, bothDone = false, myDone = false } = {}) {
    if (riskLevel === 'high' || riskLevel === 'watch') return 'watch';
    if (hasReport || bothDone) return 'repair';
    if (myDone) return 'calm';
    return 'calm';
}

function buildHomePosterTitle({ hasReport = false, bothDone = false, myDone = false, riskLevel = 'none', solo = false } = {}) {
    if (riskLevel === 'high' || riskLevel === 'watch') return solo ? '先缓下来。' : '先缓下来。';
    if (hasReport) return solo ? '先读懂，再往下走。' : '先读懂，再靠近。';
    if (bothDone) return '判断已经开始成形。';
    if (myDone) return '你已经先走了一步。';
...
function buildHomePosterLead({ hasReport = false, bothDone = false, myDone = false, latestInsight = '', nextAction = '', solo = false } = {}) {
    if (hasReport) return trimHandmadeCopy(latestInsight, 24, solo ? '先回看今天。' : '先把主结论读清楚。');
    if (bothDone) return '材料已经够了，判断会开始成形。';
    if (myDone) return solo ? '你已经开始了，接着补一句就够。' : '你已经先走了一步，现在先别急着一次说完。';
...
function renderReportEvidenceLine(timelineData = {}, evidenceSummary = []) {
    const events = Array.isArray(timelineData?.events) ? timelineData.events.slice(0, 3) : [];
    const items = events.length
        ? events.map((event) => ({
            ...
            title: event?.title || event?.headline || event?.summary || '关系事件',
            meta: event?.impact_summary || event?.description || event?.next_step || '',
        }))
...
function renderHandmadeReportHero(report, options = {}) {
    ...
    const actionButtons = isSolo
        ? `<button class="button button--secondary" type="button" onclick="showPage('checkin')">今日记录</button>
           <button class="button button--ghost" type="button" onclick="showPage('timeline')">时间轴</button>`
        : `<button class="button button--secondary" type="button" onclick="openMessageSimulator()">预演</button>
...
function renderHome(payload) {
    ...
    const scene = resolveEditorialScene({
        ...
        hasReport: Boolean(payload.todayStatus?.has_report),
        bothDone: Boolean(payload.todayStatus?.both_done),
        myDone: Boolean(payload.todayStatus?.my_done),
    });
    ...
    const secondaryAction = payload.todayStatus?.has_report ? "showPage('timeline')" : payload.todayStatus?.my_done ? "openCheckinMode('voice')" : "showPage('discover')";
    const secondaryLabel = payload.todayStatus?.has_report ? '打开时间线' : payload.todayStatus?.my_done ? '语音补一句' : '看总览';
    const currentStateLine = payload.todayStatus?.has_report
        ? '这一期已经可以读了。'
    ...
    const riverItems = timelineItems.length ? timelineItems : [
        {
            date: payload.todayStatus?.my_done ? '今天' : '现在',
            title: payload.todayStatus?.my_done ? '你已经把自己的这一句留下了' : '先把今天这一句留下来',
            meta: payload.todayStatus?.my_done ? '接下来不用一次谈完全部问题，先看系统会把什么整理出来。' : '哪怕只写一句，也比空白更有用。',
        },
        {
            date: payload.todayStatus?.has_report ? '随后' : '接着',
            title: payload.todayStatus?.has_report ? '这一期简报已经可读' : payload.todayStatus?.both_done ? '系统会开始生成判断' : '等材料继续补齐',
            meta: payload.todayStatus?.has_report ? trimHandmadeCopy(latestInsight, 42, '现在最适合先把主结论读清楚。') : trimHandmadeCopy(nextAction, 42, '页面会继续沿着这些材料推断。'),
        },
        {
            date: '之后',
...
function renderReport(report, history, trendData, options = {}) {
    ...
    if (report?.status === 'pending') {
        ...
        const scene = resolveEditorialScene({
            riskLevel: options.safetyStatus?.risk_level || content.crisis_level || 'none',
            hasReport: true,
            bothDone: true,
            myDone: true,
        });
        const actionButtons = isSolo
            ? '<button class="button button--primary" type="button" onclick="showPage(\'checkin\')">回到今日记录</button><button class="button button--ghost" type="button" onclick="showPage(\'timeline\')">打开时间轴</button>'
            : '<button class="button button--primary" type="button" onclick="openMessageSimulator()">聊天前预演</button><button class="button button--ghost" type="button" onclick="showPage(\'timeline\')">打开时间轴</button>';
        ...
        <section class="cockpit-shell cockpit-shell--evidence">
            ...
            <div class="cockpit-grid">
                <div class="cockpit-grid__main">
                    ...
                    ${isDemoMode()
                        ? '<section class="safety-panel safety-panel--compact"><div class="safety-panel__head"><div><p class="panel__eyebrow">查看模式</p><h4>样例模式不会改真实数据</h4></div></div><p class="safety-panel__summary">当前页面只展示结构和内容，不开放改名、解绑或策略调整等修改操作。</p></section>'
                        : ''}
                </div>
                <div class="cockpit-grid__side">
                    ${isDemoMode()
                        ? '<section class="cockpit-shell cockpit-shell--actions"><div class="cockpit-shell__head"><div><p class="panel__eyebrow">策略概览</p><h4>策略面板（查看）</h4></div></div><p>样例模式下只展示策略内容，不触发新增、编辑、启停或回滚。</p><div class="cockpit-action-row"><button class="button button--ghost" type="button" onclick="exitDemoMode()">退出样例</button></div></section>'
