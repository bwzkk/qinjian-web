...
function buildApiRootCandidates() {
  var _a;
  const candidates = [];
  const explicitRoot = (_a = window.QJ_CONFIG) == null ? void 0 : _a.apiRoot;
  if (explicitRoot) {
...
class ApiClient {
  ...
  async requestWithTimeout(method, path, body = null, timeoutMs = 12e3) {
  ...
  async getDailyTasks(pairId) {
    return this.requestWithTimeout("GET", `/tasks/daily/${pairId}`, null, 8e3);
  }
  ...
  async buildRealtimeAsrSocketUrl() {
    ...
    const absoluteApiRoot = connection.apiRoot.startsWith("http") ? connection.apiRoot : `${window.location.origin}${connection.apiRoot.startsWith("/") ? "" : "/"}${connection.apiRoot}`;
  ...
  async getAdminPrivacyAudits(filters = {}) {
    ...
    if (filters.pairId)
      params.set("pair_id", filters.pairId);
    if (filters.since)
